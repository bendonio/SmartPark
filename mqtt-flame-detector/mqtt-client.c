...
{
  static bool connected_to_mqtt_broker = false;
  ...
  while(1) {
    ...
    if((ev == PROCESS_EVENT_TIMER && data == &flame_detector_state_check_timer) || 
        ...
        if(status == MQTT_STATUS_ERROR) {
          LOG_ERR("Error while connecting to the MQTT broker, retrying!\n");
          state = STATE_NET_OK;
        }else {
          LOG_DBG("No error in mqtt_connect, state = STATE_CONNECTING!\n");
      ...
      etimer_set(&flame_detector_state_check_timer, FLAME_DETECTOR_STATE_CHECK_INTERVAL);
        ...
        if(reset_btn->press_duration_seconds > 5) {
          ...
          // TODO: send a message to the collector to make it know that the alarm is off.
