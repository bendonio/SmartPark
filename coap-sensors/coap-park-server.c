...
#include "dev/leds.h"
#include "./resources/res-presence-sensor.h"
...
// TODO: move all these constants in a dedicated file
...
void activate_resources(void){
  ...
  coap_activate_resource(&res_presence_sensor, "slotState");
      ...
      if(etimer_expired(&check_network_timer)){
        ...
        if(!have_connectivity()){
          ...
          if(!registered || !registration_sent){
            ...
            if(isOccupied())
              sprintf(payload, "{\"parkSlotID\": \"%s\", \"occupied\": true}", ipv6_addr);
            else
              sprintf(payload, "{\"parkSlotID\": \"%s\", \"occupied\": false}", ipv6_addr);
