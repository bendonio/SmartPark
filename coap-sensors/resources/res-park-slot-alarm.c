...
/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "CoAP Park Slot Alarm Resource"
#define LOG_LEVEL LOG_LEVEL_APP

static void res_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset);

/* A simple actuator example, depending on the color query parameter and post variable mode, corresponding led is activated or deactivated */
RESOURCE(res_alarm_system,
         "title=\"Alarm System\";rt=\"alarmSystem\"",
         NULL,
         NULL,
         res_put_handler,
         NULL);
...
res_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset){
    ...
    if(strcmp(COMMAND_START_ALARM, (char*)request->payload) == 0){
        ...
        LOG_DBG("Sending PARK_SERVER_FIRE_ALARM_START_EVENT to coap_park_server process!\n");
        process_post(&coap_park_server, PARK_SERVER_FIRE_ALARM_START_EVENT, NULL);
